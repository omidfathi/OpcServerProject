...
async def main():
    ...
    server.set_endpoint("opc.tcp://0.0.0.0:4850/freeopcua/server/")
    ...
    mydevice_var = await mydevice.get_child(
        [f"{idx}:controller", f"{idx}:state"])  # get proxy to our device state variable
    ...
    multiply_node = await myobj.add_method(idx, "multiply", multiply, [ua.VariantType.Int64, ua.VariantType.Int64],
    ...
    async with server:
        ...
        await server.write_attribute_value(myvar.nodeid, ua.DataValue(
            0.9))  # Server side write method which is a bit faster than using write_value
        while True:
...
if __name__ == "__main__":
    ...
    asyncio.SelectorEventLoop().run_until_complete(asyncio.wait([mqtt_set(), main()]))
